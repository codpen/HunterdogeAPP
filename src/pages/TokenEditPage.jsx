			...
			<Stack className={classes.container}>
					...
					GeneralLinks.map((item, index) =>
						<Stack key={index} className={classes.inputGroup}>
							<Typography>{item.label}</Typography>
							{
								item.isTextarea
									?
									<Stack component='textarea' defaultValue={generalInfo[item.name]} onChange={(e) => { handleChangeGeneralInfo(e.target.value, item.name) }}></Stack>
									:
									<Input value={generalInfo[item.name]} onChange={(e) => { handleChangeGeneralInfo(e.target.value, item.name) }}></Input>
							}
						</Stack>
					)
				}
				<Text size={'0.8em'} margin={'20px auto 10px auto'} font={'italic'}>2. Tokenomics</Text>
				{
					TokennoicsLinks.map((item, index) =>
						<Stack key={index} className={classes.inputGroup}>
							<Typography>{item.label}</Typography>
							<Input value={tokenomics[item.name]} onChange={(e) => { handleChangeTokenomics(e.target.value, item.name) }}></Input>
						</Stack>
					)
				}
				...
				</Flex>
				{
					openPresale && PresaleLinks.map((item, index) =>
						<Stack key={index} className={classes.inputGroup}>
							<Typography>{item.label}</Typography>
							<Input value={presales[item.name]} onChange={(e) => { handleChangePresales(e.target.value, item.name) }}></Input>
						</Stack>
					)
				}
			</Stack>
